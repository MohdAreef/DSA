...
The span of the stock's price in one day is the maximum number of consecutive days (starting from that day and going backward) for which the stock price 
...
class StockSpanner {
    ...
    public int next(int price) {
        ...
        if(stack.isEmpty())
        {
        stack.push(new Pair(price,timer));
             return timer;
        }
        else
        {
            int ans=timer-stack.peek().index;
            stack.push(new Pair(price,timer));
            return ans;
        }
    }
